...
INSTALL_PAGE     = 'http://tmbkodi.com/signup/check.php'
...
def suminstall(redo=False):
	if wiz.getS('login') == '' or wiz.getS('pass') == '' or redo == True:
		if redo == True: choice = 1
		else: choice = DIALOG.yesno(ADDONTITLE, "You are required to login to install builds from %s!" % ADDONTITLE, "Would you like to enter in your information?", "Visit %s to sign up!" % MEMBERS_URL, yeslabel="Enter Details", nolabel="No Cancel")
		if choice == 1: 
			login    = wiz.Get_Keyboard(wiz.getS('login'), 'Enter Your Login Email')
			password = wiz.Get_Keyboard(wiz.getS('pass'), 'Enter Your Password')
			wiz.setS('login', login)
			wiz.setS('pass', password)
		else:
			sys.exit()
	url = "%s?email=%s&pass=%s" % (INSTALL_PAGE, wiz.getS('login'), wiz.getS('pass'))
	link  = wiz.openURL(url).replace('\n', '').replace('\r', '')
	wiz.log(str(link))
	total = re.compile('install="(\d)"').findall(link)
	try:
		if (str(total[0]) == '1'):
			wiz.log("Login Successful!")
	except: 
		if DIALOG.yesno('Authentication Failure', 'Please signup here: https://tmbkodi.com', 'Then Re-Enter the details', yeslabel="Re-Enter Details", nolabel="No Cancel"):
			suminstall(True)
		else:
			sys.exit()
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart in match:
		...
		addFile('===============[ WIPE + INSTALL ]===================', '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('[B]CLICK TO INSTALL[/B]'   , 'install', name, 'fresh'   , fanart=fanart, icon=icon, themeit=THEME1)
		addFile('===============[ NO WIPE + INSTALL ]===================', '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('[B]CLICK TO UPDATE[/B]', 'install', name, 'normal'  , fanart=fanart, icon=icon, themeit=THEME1)
