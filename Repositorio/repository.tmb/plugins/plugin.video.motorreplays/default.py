import xbmc,xbmcaddon,xbmcgui,xbmcplugin,urllib,urllib2,os,re,sys,datetime,shutil,resolveurl,urlresolver,random,liveresolver
...
AddonTitle          = '[COLOR yellow]Motor Replays[/COLOR]'
...
baseurl         = 'http://supremacy.org.uk/Motor%20Replays/base.xml'
...
messagetext     ='http://supremacy.org.uk/Motor%20Replays/info.xml'
startinfo       ='http://supremacy.org.uk/Motor%20Replays/startinfo.xml'
...
def GetMenu():
     ...
     url = baseurl
     addDir('[B][COLOR yellow]News & Updates[/COLOR][/B]',url,10,whatsnewicon,fanarts)
...
def popup():
        ...
        if len(message)>1:
                ...
                if compfile == message:pass
                        ...
                        showText('[B][I][COLOR yellow]Motor Replays  ~  Latest  ~  Content Updates[/COLOR][/I][/B]', message)
...
def NEW():
        ...
        if len(message)>1:
                path = xbmcaddon.Addon().getAddonInfo('path')
                showText('[B][I][COLOR yellow]Motor Replays  ~  News & Updates[/COLOR][/I][/B]', message)
                quit()
...
def REDDIT_SUGGESTED():
    ...
    r = open_url('http://supremacy.org.uk/Motor%20Replays/suggested.xml')
...
def EVENT_REDDIT():
    ...
    r = open_url('http://supremacy.org.uk/Motor%20Replays/events.xml')
    r = re.compile('<link>(.+?)</link>').findall(r)
...
def REDDIT_PLAYER(name,url,iconimage):
    ...
    dp.create(AddonTitle,"[COLORyellow]DRIVERS... [/COLOR]",'[COLOR yellow]START YOUR ENGINES!!! [/COLOR]','')   
    dp.update(0)
    import resolveurl
    import liveresolver
...
def SEARCH():
	keyb = xbmc.Keyboard('', '[COLOR yellow]Search ~ Motor Replays[/COLOR]')
	keyb.doModal()
	if (keyb.isConfirmed()):
		searchterm=keyb.getText()
		searchterm=searchterm.upper()
	else:quit()
	link=open_url('http://supremacy.org.uk/Motor%20Replays/search.xml')
