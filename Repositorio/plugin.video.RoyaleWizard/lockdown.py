...
try:
      ...
      addon_id = 'plugin.video.Royale'
      selfAddon = xbmcaddon.Addon(id=addon_id)
      comparefile = os.path.join(os.path.join(path,''), 'compare.txt')

      #modify xml for uncached thumbnails
      xmlpath = os.path.join(path,'resources','skins','lockdown','720p','lockdown.xml')
      ...
      xml=xml.replace(fanart,'https://i.imgur.com/7DY5bB8.jpg?'+str(rand)).replace(icon,'https://i.imgur.com/7DY5bB8.jpg?'+str(rand))
      xml_file = open(xmlpath, "w");xml_file.write(xml);xml_file.close()
      #modify xml for uncached thumbnails

      req = urllib2.Request('http://lockdownspmc.com/Notification/message.txt?'+str(rand))
      ...
      if compfile == text:quit()
            ...
            win = window.lockdownWindow('lockdown.xml',path,'lockdown',caption='',text=text)
            xbmc.sleep(1000)
            win.doModal()
            del win
