...
def speedtest(list = False , mini = None , server = None , share = True , simple = False , src = None , timeout = 10 , units = ( 'bit' , 8 ) , version = False):
	...
	dp.create("[COLOR gold]Test de velocidad[/COLOR]", "[COLOR white]Iniciando test..[/COLOR]", "", "")
	dp.update(0)
	...
	dp.update(4, "", "", "[COLOR white]Recuperando lista de servidores speedtest.net...[/COLOR]")

	servers = closestServers(config['client'])

	print_('Testing from %(isp)s (%(ip)s)...' % config['client'])
	dp.update(6, '[COLOR white]Tu red es: %(isp)s (%(ip)s)...[/COLOR]' % config['client'])

	print_('Selecting best server based on latency...')
	#dp.update(8, "", "", "[COLOR white]Selecting best server based on latency...[/COLOR]")
	best = getBestServer(servers)

	print_(('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
			'%(latency)s ms' % best).encode('utf-8', 'ignore'))
	
	#dp.update(10, "", ('[COLOR white]Server location: %(name)s [%(d)0.2f km]: [/COLOR]'
	#		'[COLOR white]%(latency)s ms[/COLOR]' % best).encode('utf-8', 'ignore'))	
	...
	dp.update(15, "", "", "[COLOR white]Velocidad de bajada...[/COLOR]")
	...
	print_('[COLOR white]Testing upload speed[/COLOR]', end='')
	dp.update(65, "", "", "[COLOR white]Velocidad de subida...[/COLOR]")
	...
	while ulspeed < 1:
		...
		dp.update(65, "", "", "[COLOR white]Testing upload speed... [Attempt [/COLOR]" + str(i) + "]")
		ulspeed = uploadSpeed(best['url'], sizes, simple)
		print_()
		print_('Upload: %0.2f M%s/s' %
		   ((ulspeed / 1000 / 1000) * units[1], units[0]))
	...
	dp.update(95, "", "", "[COLOR white]Listando resultados...[/COLOR]")
