...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        try: 
            ...
            e = 'Velocidad: %.02f Kb/s ' % kbps_speed 
            e += 'Tiempo: %02d:%02d' % divmod(eta, 60)
            lockdownstring = '[COLOR lime]Royale esta trabajando...[/COLOR]'
            dp.update(percent, mbs, e,lockdownstring)
        except: 
            percent = 100 
            dp.update(percent) 
        if dp.iscanceled(): 
