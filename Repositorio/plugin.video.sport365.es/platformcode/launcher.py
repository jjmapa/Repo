...
def start():
    ...
    """
    logger.info("sport365-es.platformcode.launcher start")
...
def run():
    logger.info("sport365-es.platformcode.launcher run")
    ...
    if item.action != "actualiza":
        logger.info("sport365-es.platformcode.launcher "+item.tostring())
    ...
    try:
        ...
        if item.action == "":
            logger.info("sport365-es.platformcode.launcher Item sin accion")
            return
        ...
        if item.action == "getmainlist":
            ...
            if config.get_setting("updatecheck") == "true":
                ...
                try:
                    ...
                    logger.info("sport365-es.platformcode.launcher Fallo al verificar la actualización")

            else:
                logger.info("sport365-es.platformcode.launcher Check for plugin updates disabled")
            ...
            file_keyboard = xbmc.translatePath("special://profile/keymaps/sport365-es.xml")
            if config.get_setting("matchcenter_enabled") == "true" and not os.path.exists(file_keyboard):
                ...
                data = '<keymap><global><keyboard><key id="%s">'  % tecla + 'runplugin(plugin://plugin.video.sport365-es/?ewogICAgImFjdGlvbiI6ICJzdGFydCIsIAogICAgImNoYW5uZWwiOiAibWF0Y2hjZW50ZXIiLCAKICAgICJpbmZvTGFiZWxzIjoge30KfQ%3D%3D))</key></keyboard></global></keymap>'
        ...
        # Action in certain channel specified in "action" and "channel" parameters
            ...
            logger.info("sport365-es.platformcode.launcher channel_file=%s" % channel_file)
            ...
            logger.info("sport365-es.platformcode.launcher running channel "+channel.__name__+" "+channel.__file__)

            # Special play action
            if item.action == "play":
                logger.info("sport365-es.platformcode.launcher play")
                # logger.debug("item_toPlay: " + "\n" + item.tostring('\n'))

                # First checks if channel has a "play" function
                if hasattr(channel, 'play'):
                    logger.info("sport365-es.platformcode.launcher executing channel 'play' method")
                ...
                # If player don't have a "play" function, not uses the standard play from platformtools
                    ...
                    logger.info("sport365-es.platformcode.launcher executing core 'play' method")
                    platformtools.play_video(item)
            ...
            # Special action for findvideos, where the plugin looks for known urls
                ...
                # If not, uses the generic findvideos function
                    ...
                    logger.info("sport365-es.platformcode.launcher no channel 'findvideos' method, "
            ...
            # Special action for searching, first asks for the words then call the "search" function
                ...
                logger.info("sport365-es.platformcode.launcher search")
            ...
            # For all other actions
                ...
                logger.info("sport365-es.platformcode.launcher executing channel '"+item.action+"' method")
                itemlist = getattr(channel, item.action)(item)
                platformtools.render_items(itemlist, item)

    except urllib2.URLError, e:
        import traceback
        logger.error("sport365-es.platformcode.launcher "+traceback.format_exc())

        # Grab inner and third party errors
        if hasattr(e, 'reason'):
            logger.info("sport365-es.platformcode.launcher Razon del error, codigo: "+str(e.reason[0])+", Razon: " +
                        str(e.reason[1]))
        ...
        # Grab server response errors
            ...
            logger.info("sport365-es.platformcode.launcher codigo de error HTTP : %d" % e.code)
        ...
        if canal:
            platformtools.dialog_ok(
                ...
                "ha cambiado su estructura, o un error interno de sport365-es.",
