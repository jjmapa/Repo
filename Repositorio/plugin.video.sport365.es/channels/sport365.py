...
#
# Canal sport365 creado por deportesalacarta adaptado para Sport365.live
#
...
def entradas(item):
    ...
    for i, f in enumerate(fechas):
        ...
        for url, thumb, hora, title, datos, deporte in matches:
            ...
            if len(fechas) == 1:
                title = "[COLOR %s]%s - %s [/COLOR][COLOR orange](%s)[/COLOR]" % (text_color, hora, title, datos)
            else:
...
def findvideos(item):
    ...
    for i, url in enumerate(matches):
        ...
        title = "[COLOR green]Stream %s - [/COLOR][COLOR orange](%s)[/COLOR]" % (i+1, item.info)
