...
def read(path, linea_inicio=0, total_lineas=None):
    ...
    if path.lower().startswith("smb://"):
        ...
        try:
            ...
            logger.info("sport365.es.core.filetools read: ERROR al leer el archivo: {0}".format(path))
        ...
        try:
            ...
            logger.info("sport365.es.core.filetools read: ERROR al leer el archivo: %s" % path)
...
def write(path, data):
    ...
    if path.lower().startswith("smb://"):
        ...
        try:
            ...
            logger.info("sport365.es.core.filetools write: Error al guardar el archivo: {0}".format(path))
            return False
        else:
            return True
...
def rename(path, new_name):
    ...
    if path.lower().startswith("smb://"):
        ...
        try:
            ...
            import traceback
            logger.info(
                "sport365.es.core.filetools mkdir: Error al renombrar el archivo o carpeta" + traceback.format_exc())
...
def exists(path):
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            logger.info("sport365.es.core.filetools exists: No es posible conectar con la ruta")
...
def remove(path):
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            import traceback
            logger.info("sport365.es.core.filetools mkdir: Error al eliminar el archivo " + traceback.format_exc())
...
def rmdir(path):
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            import traceback
            logger.info("sport365.es.core.filetools mkdir: Error al eliminar el directorio " + traceback.format_exc())
...
def mkdir(path):
    ...
    @return: devuelve False en caso de error
    """
    logger.info("sport365.es.core.filetools mkdir " + path)
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            import traceback
            logger.info("sport365.es.core.filetools mkdir: Error al crear la ruta " + traceback.format_exc())
...
def remove_tags(title):
    ...
    """
    logger.info("sport365.es.core.filetools remove_tags")
