...
REMOTE_VERSION_FILE = "https://raw.githubusercontent.com/Andorth/netai/master/sport365.es.live/version"
LOCAL_XML_FILE = os.path.join(config.get_runtime_path() , "version.xml" )


def check():
    logger.info("sport365.es-live.channels.update_sports Comprobando versión")
    try:
        ...
        if float(version_local) > 1.15:
            ...
            logger.info("sport365.es-live.channels.update_sports Versión en el repositorio: %s" % version_publicada)

        logger.info("sport365.es-live.channels.update_sports Versión local: %s" % version_local)
        if float(version_publicada) > float(version_local):
            logger.info("sport365.es-live.channels.update_sports Nueva versión encontrada")
            return True, version_publicada, message, url_repo, server
        else:
            logger.info("sport365.es-live.channels.update_sports No existe versión actualizada")
            return False, "", "", "", ""
    except:
        import traceback
        logger.error("sport365.es-live.platformcode.launcher "+traceback.format_exc())
        return False, "", "", "", ""


def actualiza(item):
    logger.info("sport365.es-live.channels.update_sports actualiza")
    ...
    filename = 'sport365.es-live-%s.zip' % item.version
    ...
    try:
        ...
        # Lo descomprime
        logger.info("sport365.es-live.channels.configuracion descomprime fichero...")
        from core import ziptools
        unzipper = ziptools.ziptools()
        logger.info("sport365.es-live.channels.configuracion destpathname=%s" % local_folder)
    ...
    # Borra el zip descargado
    logger.info("sport365.es-live.channels.configuracion borra fichero...")
    try:
        filetools.remove(localfilename)
    except:
        pass
    logger.info("sport365.es-live.channels.configuracion ...fichero borrado")
...
def do_download(url, localfilename):
    ...
    logger.info("sport365.es-live.channels.update_sports localfilename=%s" % localfilename)
    logger.info("sport365.es-live.channels.update_sports url=%s" % url)
    logger.info("sport365.es-live.channels.update_sports descarga fichero...")
    inicio = time.clock()
    ...
    try:
        ...
        outfile.write(data)
        outfile.close()
        logger.info("sport365.es-live.channels.update_sports Grabado a " + localfilename)
         
    except:
        logger.info("sport365.es-live.channels.update_sports Error al grabar " + localfilename)
        import sys
    ...
    fin = time.clock()
    logger.info("sport365.es-live.channels.update_sports Descargado en %d segundos " % (fin-inicio+1))
    return error
