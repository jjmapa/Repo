...
def index():
	...
	addFile ('[I]- El mal uso puede provocar errores en el sistema[/I] ' ,'grupo', icon=ICONSPEED, themeit=THEME3)

#	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	
	addFile ('Test de Velocidad' ,'speed', icon=ICONSPEED, themeit=THEME1)
	
	addFile ('[I]- Consulta la velocidad de tu conexion[/I] ' ,'grupo', icon=ICONSPEED, themeit=THEME3)
...
def viewLogFile():
	...
	if not oldlog == False and not mainlog == False:
		...
		wiz.LogNotify('[COLOR %s]Ver Log[/COLOR]' % COLOR1, '[COLOR %s]No Log File Found![/COLOR]' % COLOR2)
		return
