...
addon = xbmcaddon.Addon('plugin.video.corban')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.corban-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(corban,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(corban,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(live.Cine.Luna,New source added.,5000,"+icon+")")
        if not url is None:
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(corban,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('corban','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('corban', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from corban Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to corban Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from corban Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to corban Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    line1 = "Este Addon es gratuito, CORBAN, no contiene ningún archivo con Copyright en su contenido. Los enlaces son recolectados de internet, razón por la cual CORBAN, no se hace responsable de dicho contenido, ni del uso no responsable del mismo. El copyright de la imágenes pertenecen a sus respectivos autores y/o productoras / distribuidores.Este Addon es gratuito, CORBAN, no contiene ningún archivo con Copyright en su contenido. Los enlaces son recolectados de internet, razón por la cual CORBAN, no se hace responsable de dicho contenido, ni del uso no responsable del mismo. El copyright de la imágenes pertenecen a sus respectivos autores y/o productoras / distribuidores. Corban se reserva los derechos de publicación de los elementos de recolectados de internet y restrasmisión.Este Addon es gratuito, CORBAN, no contiene ningún archivo con Copyright en su contenido. Los enlaces son recolectados de internet, razón por la cual CORBAN, no se hace responsable de dicho contenido, ni del uso no responsable del mismo. El copyright de la imágenes pertenecen a sus respectivos autores y/o productoras / distribuidores. CORBAN se reserva los derechos de publicación de los elementos de recolectados de internet y restrasmisión.Este Addon es gratuito, CORBAN, no contiene ningún archivo con Copyright en su contenido. Los enlaces son recolectados de internet, razón por la cual CORBAN, no se hace responsable de dicho contenido, ni del uso no responsable del mismo. El copyright de la imágenes pertenecen a sus respectivos autores y/o productoras / distribuidores. CORBAN se reserva los derechos de publicación de los elementos de recolectados de internet y retrasmisión."
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(corban,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(corban,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(corban,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(corban,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(corban,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
