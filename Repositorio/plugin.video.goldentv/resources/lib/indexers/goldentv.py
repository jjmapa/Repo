...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'http://frepo.org/main/goldenmenu.xml'
            self.list = self.goldentv_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.goldentv_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.goldentv_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            link = 'http://kodideveloper.com/Addon/search.xml'
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['goldentv']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def goldentv_list(self, url, result=None):
        try:
