...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://pastebin.com/raw/xLgE2TgN'
            self.list = self.tom_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.tom_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.tom_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.tom_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['tom']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def tom_list(self, url, result=None):
        try:
