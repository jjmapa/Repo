...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.piranha.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.piranha.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.piranha.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.piranha.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.piranha.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.piranha.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
