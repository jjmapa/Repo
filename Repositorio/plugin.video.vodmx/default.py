...
addon = xbmcaddon.Addon('plugin.video.vodmx')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.vodmx-%s]: %s" %(addon_version, string))

def makeRequest(url, headers=None):
        try:
            if headers is None:
                headers = {'User-Agent' : 'VODMX2.0.6'}
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(VODMX,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(VODMX,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
                ...
                if len(sources) > 1:
                    ...
                    if len(sources) == 1:
                        if isinstance(sources[0], list):
                            datos = getData(sources[0][1].encode('utf-8'),FANART)                            
                        else:
                            datos = getData(sources[0]['url'], sources[0]['fanart'])    
...
def addSource(url=None):        
        ...
        xbmc.executebuiltin("XBMC.Notification(VODMX,New source added.,5000,"+icon+")")
        if not url is None:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(VODMX,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(VODMX,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(VODMX,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):        
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('VODMX','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('VODMX', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from VODMX Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to VODMX Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
            ...
            mode = '12'        
        elif url.startswith('plugin://plugin.video.vodmx/'):
            if url=='plugin://plugin.video.vodmx/?mode=59':            
                isFolder=True
                mode = '59'
            elif url[33:].split('&')[1]=='mode=60':        
                mode = '60'
                isFolder=True
            elif url[33:].split('&')[1]=='mode=70':               
                mode = '70'
                isFolder=False
        else:
            mode = '12'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from VODMX Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to VODMX Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
def set_counter(url):        
        try:            
            record_url = "http://vodmx.westus2.cloudapp.azure.com/site/records.php?item="+urllib.quote_plus(url)            
            data = makeRequest(record_url)                     
        except:
            addon_log('counter failed')            
            return
        ...
        try:
                ...
                for indiv in items:
                        if re.search('name',indiv):
                            ...
                            pageurl = get_siteurl(url) + requester.parseDOM(indiv, 'a', attrs = {'class': 'name'}, ret = 'href')[0]
...
def list_items(url,query=None,result=None,content_type=None):
        ...
        try:
            thumb = requester.parseDOM(result, 'meta', ret='content', attrs = {'property': 'og:image'})[0]
        except:
            thumb = None
        
        for indiv in items:
                name = requester.parseDOM(requester.parseDOM(indiv, 'div', attrs = {'class': 'name'}), 'a')[0].encode('utf-8')
                size = requester.parseDOM(requester.parseDOM(indiv, 'div', attrs = {'class': 'size'}), 'p')[0].encode('utf-8')
                pageurl = get_siteurl(url) + requester.parseDOM(requester.parseDOM(indiv, 'div', attrs = {'class': 'name'}), 'a', ret = 'href')[0]
                ...
                list.append({'name': urllib.quote_plus(name), 'pageurl':pageurl.split('/')[5]})                
        return list

def open_folder(url, row, page="1"):
        formating=''
        headers={'Accept':'*/*','Accept-Encoding':'gzip,deflate','Connection':'keep-alive','X-Requested-With':'XMLHttpRequest'}
        if len(url.split('/')) > 4:
        ...
        if checkvalid(result):
                ...
                url_get = 'http://vodmx.westus2.cloudapp.azure.com/site/g3t_s3r13s_xm1.php?list=' + str(json.dumps(list)) + '&row='+row+'&page='+page                     
        return url_get
...
def resolve_repo(url,episodio,play=False):
        import requests
        headers={'Accept':'*/*','Accept-Encoding':'gzip,deflate','Connection':'keep-alive','X-Requested-With':'XMLHttpRequest'}
        result = requester.request(url,headers=urllib.urlencode(headers))         
        result = result.decode('iso-8859-1').encode('utf-8')        
        ...
        formurl = get_siteurl(url) + requester.parseDOM(result, 'form', ret='action', attrs = {'class': 'download_form'})[0]
        ...
        if result['DownloadUrl'].startswith('http'):
                if play==True: play_url(result['DownloadUrl'],episodio,original_url=url,original_filename=name)
...
#        print 'Not setting setResolvedUrl'
    ...
    textToSearch = get_keyboard("","Buscar",False)
    xbmc.executebuiltin('ActivateWindow(Videos,"plugin://plugin.video.vodmx/?url=http%3A%2F%2Fvodmx.westus2.cloudapp.azure.com%2Fsite%2Ftr43_d1sk0.php%3Fpalabra%3D'+textToSearch+'&mode=1&fanart=http%3A%2F%2Fvodmx.westus2.cloudapp.azure.com%2Flogos%2Ffanart.jpg",return)')    
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==60:
    params_url = get_params_url(url.replace('plugin://plugin.video.vodmx/',''))
    ...
    if regexs and len(regexs)>0:
        ...
        if data.startswith('http') or data.startswith('smb') or data.startswith('nfs') or data.startswith('/'):
            ...
            data=None
        #create xml here
    eps_url = open_folder(params_url['url'], row, page=page)
    getData(eps_url,"http%3A%2F%2Fvodmx.westus2.cloudapp.azure.com%2Flogos%2Ffanart.jpg",data)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==70:    
    try:
        ...
        pass
    params_url = get_params_url(url.replace('plugin://plugin.video.vodmx/',''))
    resolve_repo(params_url['url'],params_url['episodio'], play=True)
elif mode==13:
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(VODMX,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(VODMX,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(VODMX,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(VODMX,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(VODMX,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==53:
