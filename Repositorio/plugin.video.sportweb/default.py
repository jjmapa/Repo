...
parser = "        [COLOR blue][B]SportsWeb v0.0.1      [COLOR red]····[COLOR yellow]by Bad-Max[COLOR red]····[/B][/COLOR]"
...
def main_list(params):
	...
	plugintools.add_item(action="arena_dmax0",url="",title="[COLOR red][B]-ArenaVision[/B][/COLOR]",thumbnail="https://elrincondesereblog.files.wordpress.com/2015/12/2.png?w=768",fanart="http://1.bp.blogspot.com/-MY6Pz_Ucljk/Ut1FZLWUAcI/AAAAAAAAARo/bG-RAvuPKAM/s1600/futbol_dic2013_phixr.jpg",folder=True,isPlayable=False)
...
def arena_dmax0(params):
	...
	#headers = {"User-Agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36', "Referer": url_agenda_arena}
	...
	plugintools.log("**************************LINEAS: "+str(len(lineas)) + "********************************")
		...
		for item in apartados:
			...
			if cuenta == 1:  # Fecha ... 11/08/16... casi siempre, pero cuando les sale de los huevos, ponen el 1º los canales y la fecha el último
				...
				for item2 in apartados:
					#texto = plugintools.find_single_match(item2,'auto-style13">(.*?)</td').replace("SOCCER","Fútbol").replace("<br />"," / ")
					texto = texto.replace("<br />","").replace("FOOTBALL","Rugby Americano").replace("PRESEASON","Pretemporada").replace("OLYMPICS","Olimpicos").replace("BASKETBALL","Baloncesto").replace("SWIMMING","Natación")  # .decode('unicode_escape').encode('utf8')
					barra1 = texto[2:3]
					barra2 = texto[5:6]
...
def busca_aces(canal):
	...
	#headers = {"User-Agent": 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:41.0) Gecko/20100101 Firefox/41.0', "Referer": url_agenda_arena}
...
def arenazaping(params):
	...
	headers = {"User-Agent": 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:41.0) Gecko/20100101 Firefox/41.0', "Referer": url_ref_arena, "cookie": "beget=begetok"}
	r0=requests.get(url_ref_arena, headers=headers)
	data0 = r0.content
	#plugintools.log("********************DATA0: "+data0+"***********************")	

	
	grupo = plugintools.find_single_match(data0,'navigation" class="navigation(.*?)id="slider')
	...
	for item in canales:
		...
		if "http://arena" in item:
			if "first leaf" in item:
				itemprovi = plugintools.find_single_match(item,'first leaf(.*?)/a>')
				plugintools.log("********************ITEMPROVI: "+itemprovi+"***********************")	
				canalaces = plugintools.find_single_match(itemprovi,'href="(.*?)"')
				titulo = plugintools.find_single_match(itemprovi,'title="">(.*?)<')
			else:
				canalaces = plugintools.find_single_match(item,'"(.*?)"')
...
def lanza_aces(params):
	...
	ace = plugintools.find_single_match(data1,'href="acestream://(.*?)"')
...
def verliganet0(params):
	...
	for item in agenda:
		...
		for evento in eventos:
			...
			#link =  "http://vergol.com/live1/plusdeportes1.php"
			logo =""
